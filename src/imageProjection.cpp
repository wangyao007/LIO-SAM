...
public:
    ...
    */
    ...
    bool cachePointCloud(const sensor_msgs::PointCloud2ConstPtr& laserCloudMsg)
    {
        ...
        timeScanEnd = timeScanCur + laserCloudIn->points.back().time; //相差约0.1s,即激光雷达采样两帧之间没有时间间隔，为连续采样
   ...
   //relTime:laserCloudIn->points[i].time
    ...
    void projectPointCloud()
    {
        ...
        for (int i = 0; i < cloudSize; ++i)
        {
            ...
            if (range < lidarMinRange || range > lidarMaxRange) //最小为1米，最大为1000米
                continue;
            //距离图像的行  与点云中ring对应,
            ...
            if (rowIdn % downsampleRate != 0) //降采样，这里为1，不降采样
                continue;
            ...
            if (rangeMat.at<float>(rowIdn, columnIdn) != FLT_MAX) //初始所有值都被置为FLT_MAX
                continue;
